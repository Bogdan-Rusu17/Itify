...
///     As most routes and business logic will need to know what user is currently using the backend this service will be
///     the most used.
/// </summary>
public interface IUserService
{
    ...
    public Task<ServiceResponse<UserRecord>> GetUser(Guid id, UserRecord requestingUser,
    ...
    /// </summary>
    public Task<ServiceResponse<PagedResponse<UserRecord>>> GetUsers(UserPaginationQueryParams pagination,
        CancellationToken cancellationToken = default);

    /// <summary>
    ///     Login as suggested responds to a user login request with the JWT token and user information.
    /// </summary>
    public Task<ServiceResponse<LoginResponseRecord>> Login(LoginRecord login,
        CancellationToken cancellationToken = default);
    ...
    /// <summary>
    ///     UpdateUser updates an user and verifies if requesting user has permissions to update it, if the user is his own
